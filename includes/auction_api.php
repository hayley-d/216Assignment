...
class auctionApi
{
    public function createAuction( $name, $property_id, $user_email,$start_time,$end_time,$code)
    {
        global $db;
    ...
    public function updateAuction($code,$start,$end,$highest_bid,$buyer)
    {
        try{
            global $db;
            if ($code == null || $start== null || $end == null || $highest_bid == null || $buyer == null) {
                http_response_code(400);
                echo json_encode([
                    "success" => false,
                    "message" => "Auction data missing",
                    "data" => []
                ]);
                die();
            }
            ...
            $update_query = "UPDATE auctions SET start = ?, end = ?, highest_bid = ?, buyer = ?, status = ? WHERE auction_code = ?";
            $stmt = $db->prepare($update_query);
            $stmt->bind_param("ssdiss", $start, $end, $highest_bid, $userId, $status, $code);
            $stmt->execute();
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $response = array(
                    "status" => "Success",
                    "timestamp" => $timestamp,
                    "data" => 'Auction successfully updated'
                );
                echo json_encode($response, JSON_PRETTY_PRINT);
                die();
            } else {
        ...
        try{
            ...
            if ($stmt->affected_rows > 0) {
                ...
                $response = array(
                    "status" => "Success",
                    "timestamp" => $timestamp,
                    "data" => 'Auction successfully ended.'
                );
                echo json_encode($response, JSON_PRETTY_PRINT);
                die();
            }
...
if (!isset($_SERVER['HTTP_X_REQUESTED_WITH']) || strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
    ...
    if($type == 'CreateAuction')
    {
        ...
        /*create auction*/
        $api->createAuction($name,$property_id,$user_email,$start,$end,$code);
    }
